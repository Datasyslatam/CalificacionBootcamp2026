...
function App() {
  ...
  const renderHeader = () => (
    <header className="sticky top-0 z-50 backdrop-blur-xl bg-[#0f0f14]/90 border-b border-[#2a2a3a]">
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-[#D4F842] rounded-lg flex items-center justify-center text-lg font-black text-[#0f0f14]">
            SB
          </div>
          <div className="hidden sm:block">
            <h1 className="text-sm font-bold text-white tracking-tight">SuperBrix</h1>
            <p className="text-[10px] text-[#6b6b7b] uppercase tracking-wider">Reto IA 2026</p>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 py-12 relative z-10">
          ...
          <div className="mb-12">
            ...
            <h2 className="text-4xl md:text-5xl font-black text-white mb-4 leading-tight">
              Reto de Diseño<br />
              <span className="text-[#D4F842]">& Desarrollo</span>
            </h2>
            <p className="text-lg text-[#a0a0b0] max-w-2xl mb-8">
            ...
            <div className="flex flex-wrap gap-3">
              {['🤖 Inteligencia Artificial', '📱 App Móvil', '📊 Google Workspace', '⚙️ Industria 4.0'].map(tag => (
                <span key={tag} className="px-4 py-2 bg-white/5 border border-white/10 rounded-full text-sm text-[#a0a0b0] font-medium">
                  {tag}
                </span>
              ))}
            </div>
          ...
          <div className="mb-12">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-sm text-[#6b6b7b] mt-1">Seleccione su perfil para calificar</p>
              </div>
              <div className="flex gap-2">
                <button
                  onClick={() => setEditandoJurados(!editandoJurados)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors cursor-pointer ${
                    editandoJurados ? 'bg-[#D4F842] text-[#0f0f14]' : 'bg-white/5 text-[#a0a0b0] hover:bg-white/10 border border-white/10'
                  }`}
                >
                  {editandoJurados ? '✓ Listo' : '✏️ Editar'}
                </button>
                {editandoJurados && (
                  <button onClick={resetearJurados} className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-white/5 text-[#a0a0b0] hover:bg-white/10 border border-white/10 cursor-pointer">
                    🔄 Reset
                  </button>
                )}
              </div>
                ...
                return (
                  <div key={j.id} className="relative">
                    <button
                      ...
                      className={`w-full card-dark p-5 text-left cursor-pointer ${editandoJurados ? 'border-[#D4F842]/30' : ''}`}
                    >
                      {editandoJurados ? (
                        ...
                        <>
                          ...
                          <p className="text-xs text-[#6b6b7b] mt-0.5">{j.cargo}</p>
                          <div className="mt-3 flex items-center gap-2">
                            ...
                            <span className="text-[10px] text-[#6b6b7b] font-mono">{calsJurado}/{equipos.length}</span>
                          </div>
                        </>
                      )}
                    </button>
          ...
          {/* Info storage */}
          <div className="card-dark p-4 flex items-center gap-3 border-[#D4F842]/20">
            <div className="w-8 h-8 rounded-lg bg-[#D4F842]/10 flex items-center justify-center text-[#D4F842] text-sm">💾</div>
            <div>
              <p className="text-xs font-semibold text-white">Datos almacenados localmente</p>
              <p className="text-[10px] text-[#6b6b7b]">Las calificaciones persisten en su navegador. Exporte resultados cuando lo necesite.</p>
            </div>
          </div>
        </div>
      </div>
  ...
  const renderContexto = () => (
    <div className="max-w-5xl mx-auto px-4 py-12">
      ...
      <h2 className="text-3xl md:text-4xl font-black text-white mb-2">
        El Desafío<br /><span className="text-[#D4F842]">Industrial</span>
      </h2>
      <p className="text-[#a0a0b0] mb-8 max-w-2xl">Transformación digital para la gestión del tiempo en órdenes de producción.</p>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="card-dark p-6">
            <h3 className="text-sm font-bold text-[#D4F842] uppercase tracking-wider mb-3">⚠️ Problemas Actuales</h3>
          ...
          <div className="card-dark p-6 border-[#D4F842]/20">
        ...
        <div className="card-dark p-6">
          <h3 className="text-sm font-bold text-[#D4F842] uppercase tracking-wider mb-4">📐 Reglas de Diseño</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {[
              ...
              <div key={item.n} className="flex items-start gap-3 p-3 bg-white/[0.02] rounded-lg border border-white/5">
                <span className="num-badge text-xs">{item.n}</span>
                <div>
                  <p className="text-sm font-semibold text-white">{item.t}</p>
                  <p className="text-xs text-[#6b6b7b]">{item.d}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="card-dark p-6">
          <h3 className="text-sm font-bold text-[#D4F842] uppercase tracking-wider mb-4">📂 Categorías Operativas IA</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {categoriasOperativas.map((cat, i) => (
              <div key={i} className="flex items-start gap-3 p-3 bg-white/[0.02] rounded-lg border border-white/5">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#6b6b7b]">{cat.descripcion}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="card-dark p-6">
          <h3 className="text-sm font-bold text-[#D4F842] uppercase tracking-wider mb-4">📏 Rúbrica de Evaluación</h3>
          <div className="space-y-3">
            {criterios.map((c, i) => (
              <div key={c.id} className="flex items-center gap-4 p-3 bg-white/[0.02] rounded-lg border border-white/5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-[#6b6b7b] truncate">{c.descripcion}</p>
                </div>
  ...
  const renderCalificar = () => (
    <div className="max-w-5xl mx-auto px-4 py-12">
      {mensajeExito && (
        <div className="mb-6 card-dark p-4 flex items-center gap-3 border-[#D4F842]/30 animate-glow">
          <div className="w-8 h-8 rounded-lg bg-[#D4F842]/20 flex items-center justify-center text-[#D4F842]">✓</div>
      ...
      {!juradoActivo && (
        <div>
          ...
          <p className="text-[#6b6b7b] mb-8">Cada jurado califica de forma independiente</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
            {listaJurados.map(j => {
              const calsJurado = calificaciones.filter(c => c.juradoId === j.id).length;
              return (
                <button key={j.id} onClick={() => seleccionarJurado(j.id)} className="card-dark p-5 text-center cursor-pointer group">
                  ...
                  <h4 className="font-bold text-white text-sm">{j.nombre}</h4>
                  <div className="mt-3">
                    <span className={`text-[10px] px-2 py-1 rounded-full font-mono ${calsJurado > 0 ? 'bg-[#D4F842]/10 text-[#D4F842]' : 'bg-white/5 text-[#6b6b7b]'}`}>
      ...
      {juradoActivo && !equipoSeleccionado && (
        <div>
          <button onClick={() => setJuradoActivo(null)} className="text-[#D4F842] text-sm mb-4 flex items-center gap-1 hover:underline cursor-pointer">
            ← Cambiar jurado
          </button>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
            <div>
              ...
              <p className="text-sm text-[#6b6b7b]">Seleccionar equipo a calificar</p>
            </div>
            <button onClick={() => setEditandoEquipos(!editandoEquipos)}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors cursor-pointer ${
                editandoEquipos ? 'bg-[#D4F842] text-[#0f0f14]' : 'bg-white/5 text-[#a0a0b0] hover:bg-white/10 border border-white/10'
              }`}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
              ...
              return (
                <div key={eq.id} className="relative">
                  <button onClick={() => { seleccionarEquipo(eq.id); if (yaCalifico(juradoActivo!, eq.id)) cargarCalificacionExistente(juradoActivo!, eq.id); }}
                    className={`w-full card-dark p-4 text-left cursor-pointer ${yaCal ? 'border-[#D4F842]/30' : ''}`}>
                    ...
                    <p className="text-[10px] text-[#6b6b7b] mt-1">{yaCal ? 'Calificado' : 'Pendiente'}</p>
                  </button>
                  {editandoEquipos && (
            ...
            {editandoEquipos && (
              <button onClick={agregarEquipo} className="card-dark p-4 border-dashed border-white/20 hover:border-[#D4F842]/50 flex items-center justify-center cursor-pointer min-h-[80px]">
                <span className="text-[#6b6b7b] text-sm font-medium">+ Agregar</span>
              </button>
            )}
          </div>
        </div>
      ...
      {juradoActivo && equipoSeleccionado && (
        <div>
          <button onClick={() => { setEquipoSeleccionado(null); setPuntuaciones({}); setComentario(''); }}
            className="text-[#D4F842] text-sm mb-6 flex items-center gap-1 hover:underline cursor-pointer">
            ← Volver a equipos
          </button>

          <div className="card-dark p-5 mb-6 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-[#D4F842]/10 flex items-center justify-center text-xl">
              ...
              <div>
                ...
                <p className="text-xs text-[#6b6b7b]">→ {equipos.find(e => e.id === equipoSeleccionado)?.nombre}</p>
              </div>
            </div>
          ...
          <div className="space-y-3">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                  <div className="flex items-start gap-3 flex-1">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#6b6b7b]">{c.descripcion}</p>
                    </div>
                  </div>
                      ...
                      <button key={n} onClick={() => actualizarPuntuacion(c.id, n)}
                        className={`w-8 h-8 rounded-md text-xs font-bold transition-all cursor-pointer ${
                          puntuaciones[c.id] === n
                            ...
                            : n <= 6 ? 'bg-amber-500/20 text-amber-400 ring-1 ring-amber-500/50'
                            : 'bg-[#D4F842]/20 text-[#D4F842] ring-1 ring-[#D4F842]/50'
                            : 'bg-white/5 text-[#6b6b7b] hover:bg-white/10 hover:text-white'
                        }`}>
                        {n}
                      </button>
                    ))}
            ...
            {criterios.every(c => puntuaciones[c.id] !== undefined) && (
              <div className="card-dark p-5 border-[#D4F842]/20 animate-glow">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-[#D4F842] font-semibold uppercase tracking-wider">Puntaje Ponderado</p>
                    <p className="text-3xl font-black text-white mt-1">
                      {criterios.reduce((acc, c) => acc + (puntuaciones[c.id] || 0) * (c.ponderacion / 100), 0).toFixed(2)}
                      <span className="text-sm text-[#6b6b7b] font-normal"> / 10</span>
                    </p>
                  </div>
            ...
            <button onClick={guardarCalificacion} disabled={!criterios.every(c => puntuaciones[c.id] !== undefined)}
              className={`w-full py-4 rounded-xl font-bold text-sm transition-all cursor-pointer ${
                criterios.every(c => puntuaciones[c.id] !== undefined)
                  ...
                  : 'bg-white/5 text-[#6b6b7b] cursor-not-allowed'
              }`}>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="card-dark p-6 max-w-sm w-full">
            <div className="text-center mb-4">
              ...
              <p className="text-sm text-[#6b6b7b] mt-1">Esta acción no se puede deshacer.</p>
            </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="card-dark p-6 max-w-sm w-full">
            <div className="text-center mb-4">
              ...
              <p className="text-sm text-[#6b6b7b] mt-1">Se eliminarán todas sus calificaciones.</p>
            </div>
            <div className="flex gap-3">
              <button onClick={() => setConfirmarBorrado(null)} className="flex-1 py-2.5 rounded-lg border border-white/10 text-white text-sm font-medium hover:bg-white/5 cursor-pointer">Cancelar</button>
  ...
  const renderResultados = () => {
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        {mostrarExportar && (
          <div className="card-dark p-4 mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-[#D4F842]/20">
            <div className="flex items-center gap-3">
              <span className="text-xl">📥</span>
              <div>
                <p className="text-sm font-semibold text-white">Exportar resultados</p>
                <p className="text-[10px] text-[#6b6b7b]">Descargue un CSV con todos los detalles</p>
              </div>
            </div>
            <div className="flex gap-2">
        ...
        {ranking.length === 0 ? (
          <div className="card-dark p-12 text-center">
            ...
            <p className="text-sm text-[#6b6b7b] mb-4">Los jurados deben comenzar a calificar.</p>
            <button onClick={() => setVista('calificar')} className="btn-lime text-sm">Ir a calificar →</button>
          </div>
        ) : (
          <>
            {/* Podio */}
            <div className="card-dark p-6 mb-8 border-[#D4F842]/10">
              <h3 className="text-sm font-bold text-[#D4F842] uppercase tracking-wider mb-6 text-center">🏆 Top 3</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {ranking.slice(0, 3).map((eq, idx) => (
                  <div key={eq.id} className={`p-5 rounded-xl text-center ${
                    idx === 0 ? 'bg-[#D4F842]/10 border border-[#D4F842]/30' :
                    ...
                    <p className="text-2xl font-black text-[#D4F842] mt-2">{eq.resultado?.promedio.toFixed(2)}</p>
                    <p className="text-[10px] text-[#6b6b7b] mt-1">{eq.resultado?.totalJurados} jurado(s)</p>
                  </div>
                ))}
              </div>
            </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-white/5">
                      <th className="text-left p-4 text-xs font-semibold text-[#6b6b7b] uppercase">#</th>
                      <th className="text-left p-4 text-xs font-semibold text-[#6b6b7b] uppercase">Equipo</th>
                      {criterios.map(c => (
                        <th key={c.id} className="text-center p-3 text-[10px] font-semibold text-[#6b6b7b] uppercase">
                          {c.nombre.split(' ').slice(0, 2).join(' ')}<br />
                          <span className="text-[#D4F842]">{c.ponderacion}%</span>
                        </th>
                      ))}
                      <th className="text-center p-4 text-xs font-semibold text-[#6b6b7b] uppercase">Total</th>
                      <th className="text-center p-4 text-xs font-semibold text-[#6b6b7b] uppercase">Jurados</th>
                    </tr>
                  </thead>
                  <tbody>
                    {ranking.map((eq, idx) => {
                      ...
                      return (
                        <tr key={eq.id} className="border-b border-white/5 hover:bg-white/[0.02]">
                          <td className="p-4 text-lg">{idx === 0 ? '🥇' : idx === 1 ? '🥈' : idx === 2 ? '🥉' : <span className="text-[#6b6b7b] text-sm">{idx + 1}</span>}</td>
                          ...
                          <td className="p-4 text-center text-xs text-[#6b6b7b] font-mono">{eq.resultado?.totalJurados}/{listaJurados.length}</td>
                        </tr>
                      );
                    })}
                  </tbody>
            ...
            {/* Detalle por jurado */}
            <h3 className="text-sm font-bold text-[#D4F842] uppercase tracking-wider mb-4">Detalle por Jurado</h3>
                    ...
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-[#6b6b7b]">{calsJurado.length} calificado(s)</p>
                        </div>
                      </div>
                      {calsJurado.length > 0 && (
                    ...
                    {calsJurado.length > 0 ? (
                      <div className="space-y-1.5">
                        {calsJurado.map(cal => {
                          ...
                          const puntaje = criterios.reduce((acc, c) => acc + (cal.calificaciones[c.id] || 0) * (c.ponderacion / 100), 0);
                          return (
                            <div key={cal.equipoId} className="flex items-center justify-between bg-white/[0.03] rounded-lg p-2">
                              <span className="text-xs text-[#a0a0b0]">{equipo?.nombre}</span>
                              <div className="flex items-center gap-2">
                      ...
                      <p className="text-xs text-[#6b6b7b] italic">Sin calificaciones</p>
                    )}
                  </div>
                );
              })}
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="card-dark p-6 max-w-sm w-full">
              <div className="text-center mb-4">
                ...
                <p className="text-sm text-[#6b6b7b] mt-1">Se eliminarán <strong className="text-white">{calificaciones.length}</strong> calificaciones.</p>
              </div>
  ...
  return (
    <div className="min-h-screen bg-[#0f0f14]">
      ...
      <footer className="border-t border-[#2a2a3a] py-6 mt-12">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <p className="text-xs text-[#6b6b7b]">SuperBrix © 2026 — Bootcamp Fábrica Digital e Inteligencia Artificial</p>
          <p className="text-[10px] text-[#4a4a5a] mt-1">Transformación Digital & IA • SENA Regional Atlántico</p>
        </div>
      </footer>
    </div>
  );
